...
export class Slider {
    constructor(container, componentAttribute, id) {
        ...
        const bottomControlElement = container.querySelector('.slider-bottom-control');
        const rightControlElement = container.querySelector('.slider-right-control');
        const leftControlElement = container.querySelector('.slider-left-control');
        const contentControlElement = container.querySelector('.slider-content');
        ...
        const contentControl = TemplateOperators.createTemplate(this.sliderTemplate, contentContext, {}, componentAttribute);
        contentControl.events.childrenShownChanges.subscribe(item => {
            ...
            const updateHeight = () => {
                if (Array.from(item.currentChildrenShown[0].target.querySelectorAll('img')).every(elm => elm.complete && elm.naturalHeight)) {
                    this.sliderTemplate.styles = { height: `${item.currentChildrenShown[0].target.offsetHeight}px` };
                }
                else {
